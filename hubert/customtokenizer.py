...
class CustomTokenizer(nn.Module):
    def __init__(
        self, hidden_size=1024, input_size=768, output_size=10000, version=0
    ):
    ...
    def load_from_checkpoint(path, map_location: MAP_LOCATION = None):
        ...
        with ZipFile(path) as model_zip:
            ...
            if file:
                old = False
                data_from_model = Data.load(
                    model_zip.read(file).decode("utf-8")
                )
            model_zip.close()
        if old:
...
class Data:
    ...
    def __init__(
        self, input_size=768, hidden_size=1024, output_size=10000, version=0
    ):
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.output_size = output_size
...
def auto_train(
    ...
    if load_model and os.path.isfile(load_model):
        ...
        model_training = CustomTokenizer.load_from_checkpoint(
            load_model, "cuda"
        )
    else:
